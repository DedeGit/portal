...
export default class XAxis {
  constructor(ctx) {
    ...
    this.isCategoriesBarHorizontal =
    ...
    if (this.isCategoriesBarHorizontal) {
  ...
  drawXAxisLabelAndGroup(
     ...
     */
    let dataPoints =
      w.config.xaxis.type === 'categories' ? w.globals.dataPoints : labelsLen
  ...
  drawXaxisInversed(realIndex) {
    ...
    let offX = 0
    if (this.isCategoriesBarHorizontal && w.config.yaxis[0].opposite) {
  ...
  getXAxisTicksPositions() {
    ...
    if (w.globals.timescaleLabels.length > 0) {
      ...
      let xCountForCategoriesCharts = xCount
      for (let i = 0; i < xCountForCategoriesCharts; i++) {
        let x1Count = xCountForCategoriesCharts
