...
export default class Data {
      ...
      if (isXString || isXDate) {
        ...
        if (isXString || cnf.xaxis.convertedCatToNumeric) {
          ...
          if (cnf.xaxis.type === 'datetime' && !isRangeColumn) {
            ...
            // a categories and not a numeric x value
            this.fallbackToCategories = true
  ...
  parseDataAxisCharts(ser, ctx = this.ctx) {
    ...
    gl.hasGroups =
      cnf.xaxis.type === 'categories' && cnf.xaxis.group.groups.length > 0
    if (gl.hasGroups) {
    ...
    for (let i = 0; i < ser.length; i++) {
      ...
      if (this.isMultiFormat()) {
        ...
        if (i === this.activeSeriesIndex && !this.fallbackToCategories) {
          gl.isXNumeric = true
        }
      } else {
        if (cnf.xaxis.type === 'datetime') {
  ...
  handleExternalLabelsData(ser) {
    ...
    if (cnf.xaxis.categories.length > 0) {
      // user provided labels in xaxis.categories prop
      gl.labels = cnf.xaxis.categories
    } else if (cnf.labels.length > 0) {
      // user provided labels in labels props
      gl.labels = cnf.labels.slice()
    } else if (this.fallbackToCategories) {
  ...
  _generateExternalLabels(ser) {
    ...
    if (cnf.xaxis.convertedCatToNumeric) {
      gl.categoriesLabels = labelArr.map((l) => {
  ...
  parseData(ser) {
    ...
    // If we detected string in X prop of series, we fallback to categories x-axis
    this.fallbackToCategories = false
    ...
    const catLabels = this.coreUtils.getCategoriesLabels(gl.labels)
